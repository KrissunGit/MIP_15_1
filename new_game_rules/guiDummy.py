...
class GameGUI:
    ...
    def start_game(self):
        ...
        # Jaunā GameState inicializācija, lai atbilstu tavai klasei
        start_turn = HUMAN if self.starter_var.get() == "Human" else COMPUTER
        self.state = GameState(elements=seq, turn=start_turn)

        self.log("=" * 40)
        # Izlabots, lai rādītu vērtības no objektiem
        self.log(f"Jauna spēle! Virkne: {[e.value for e in self.state.elements]}")
    ...
    def _refresh(self):
        ...
        can_take = [False] * len(self.state.elements)
        ...
        for i, element in enumerate(self.state.elements):
            col_frame = tk.Frame(self.numbers_frame, bg="#f0f0f0")
            col_frame.pack(side="left", padx=3)

            # --- LABOTS: Piekļūstam GameElement īpašībām ---
            val = element.value
            ...
            if val in (2, 4):
                ...
                tk.Button(col_frame, text="Split", width=5,
                          command=lambda idx=i, n=val: self._human_move(
    ...
    def _human_move(self, action, idx):
        ...
        val = self.state.elements[idx].value
        ...
        self.log(f"  Virkne: {[e.value for e in self.state.elements]}")
